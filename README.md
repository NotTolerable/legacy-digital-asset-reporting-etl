...
A small, resume-ready Python ETL project that simulates migrating legacy digital-asset trade reporting data from raw CSV files into a structured SQLite database for analytics and reporting.

The project is designed for Backend/Data Engineering Co-op interviews. It demonstrates practical ETL fundamentals: extracting CSV data, validating and standardizing legacy records, enforcing database constraints, preserving rejected records, tracking pipeline runs, and exposing SQL reporting queries.
...
## How This Simulates Legacy Pipeline and Reporting Migration
...
> I built a small Python and SQLite ETL pipeline that simulates migrating legacy digital-asset trade reports from CSV into a structured reporting database. The pipeline extracts raw trades, validates data-quality rules, standardizes legacy symbols like `BTC/USD`, calculates notional value, loads valid rows idempotently using `trade_id` as a primary key, and stores rejected rows with reasons. It also logs each pipeline run and includes a SQL reporting view for daily trading volume by symbol. This demonstrates backend/data engineering skills around ETL design, SQL modeling, data integrity, duplicate prevention, and reporting infrastructure.
